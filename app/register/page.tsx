...
export default function RegisterPage() {
    ...
    return (
        <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #0f172a 0%, #1e1b4b 50%, #0f172a 100%)', fontFamily: 'Inter, sans-serif', padding: '1rem' }}>
            <div style={{ position: 'fixed', top: '20%', right: '15%', width: 280, height: 280, background: 'rgba(139,92,246,0.15)', borderRadius: '50%', filter: 'blur(80px)', pointerEvents: 'none' }} />
            <div style={{ position: 'fixed', bottom: '20%', left: '10%', width: 220, height: 220, background: 'rgba(99,102,241,0.12)', borderRadius: '50%', filter: 'blur(70px)', pointerEvents: 'none' }} />
            ...
            <div style={{ width: '100%', maxWidth: 420, background: 'rgba(30,41,59,0.85)', backdropFilter: 'blur(20px)', borderRadius: 20, border: '1px solid rgba(255,255,255,0.1)', padding: '2.5rem', boxShadow: '0 25px 50px rgba(0,0,0,0.5)' }}>
                <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
                    <div style={{ width: 64, height: 64, background: 'linear-gradient(135deg, #8b5cf6, #6366f1)', borderRadius: 18, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: '2rem', marginBottom: '1rem', boxShadow: '0 8px 24px rgba(139,92,246,0.4)' }}>🚀</div>
                    <h1 style={{ color: '#f1f5f9', fontSize: '1.6rem', fontWeight: 700, marginBottom: '0.4rem' }}>Tạo tài khoản</h1>
                    <p style={{ color: '#94a3b8', fontSize: '0.9rem' }}>Đăng ký miễn phí để sử dụng AI Chat</p>
                </div>
                ...
                <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    ...
                    <div>
                        ...
                        <div style={{ position: 'relative' }}>
                            <input type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="Ít nhất 6 ký tự" required style={{ ...inputStyle, paddingRight: '2.8rem' }}
                                onFocus={e => e.target.style.borderColor = '#8b5cf6'} onBlur={e => e.target.style.borderColor = 'rgba(255,255,255,0.1)'} />
                    ...
                    <div>
                        <label style={{ display: 'block', color: '#cbd5e1', fontSize: '0.85rem', fontWeight: 500, marginBottom: '0.4rem' }}>Xác nhận mật khẩu</label>
                        <input type={showPass ? 'text' : 'password'} value={confirm} onChange={e => setConfirm(e.target.value)} placeholder="Nhập lại mật khẩu" required style={inputStyle}
                            onFocus={e => e.target.style.borderColor = '#8b5cf6'} onBlur={e => e.target.style.borderColor = 'rgba(255,255,255,0.1)'} />
                    </div>
                    ...
                    <button type="submit" disabled={loading} style={{ width: '100%', background: loading ? '#4b5563' : 'linear-gradient(135deg, #8b5cf6, #6366f1)', border: 'none', borderRadius: 12, padding: '0.85rem', color: '#fff', fontSize: '1rem', fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', boxShadow: loading ? 'none' : '0 4px 16px rgba(139,92,246,0.4)', marginTop: '0.25rem' }}>
                ...
                <p style={{ textAlign: 'center', color: '#64748b', fontSize: '0.88rem', marginTop: '1.5rem' }}>
                    Đã có tài khoản?{' '}
                    <Link href="/login" style={{ color: '#818cf8', fontWeight: 600, textDecoration: 'none' }}>Đăng nhập</Link>
                </p>
            </div>
        </div>
    );
